...
| 程序 | 目标模型 | 推理后端 | 默认构建 |
| --- | --- | --- | --- |
| `onnx-cli` | Qwen3.5-0.8B | ONNX Runtime（C++ API） | 是 |
| `onnx-genai-cli` | Qwen3-0.6B | ONNX Runtime GenAI | 否（需显式启用） |
...
`onnx-cli` 基于 ONNX Runtime C++ API 实现，不链接 ONNX Runtime GenAI，也不依赖 `genai_config.json`：

- Qwen ByteLevel BPE 分词与流式解码
- Chat Template 拼接（支持思考 / 非思考模式）
- 图片加载、缩放、归一化与视觉 Patch 打包
- Vision Encoder、Embedding 与 Decoder ONNX 图执行
- 视觉特征注入与多模态 MRoPE `position_ids`
- 18 组 recurrent state 与 6 组 KV Cache 的维护
- Greedy、top-k、top-p、temperature 与 presence penalty 解码策略
- EOS 检测与推理性能统计

`onnx-genai-cli` 基于 ONNX Runtime GenAI 实现，使用模型目录中的
`genai_config.json` 与 Chat Template：

- ONNX Runtime GenAI 的模型、分词器与流式生成接口
- 模型提供的 Chat Template，以及思考 / 非思考模式
- Greedy、top-k、top-p、temperature 与 repetition penalty 解码策略
- 交互式多轮输入、流式输出与推理性能统计
...
│   ├── qwen_image_processor.h  # Qwen 图片预处理接口
...
│   ├── qwen_image_processor.cpp # 图片解码与视觉 Patch 预处理
...
├── third-party/
│   └── ...                     # Git 子模块（ONNX Runtime、GenAI 与 stb）
└── CMakeLists.txt
```
...
ONNX Runtime、ONNX Runtime GenAI 与图片解码库 stb 均以 Git 子模块形式引入
`third-party/`，版本固定如下：

| 子模块 | 版本 | 简介 |
| --- | --- | --- |
| [`third-party/onnxruntime`](https://github.com/microsoft/onnxruntime) | `v1.27.1` | 跨平台 ONNX 推理引擎；`onnx-cli` 与 `inspect_onnx` 直接调用其 C++ API |
| [`third-party/onnxruntime-genai`](https://github.com/microsoft/onnxruntime-genai) | `v0.14.0` | 基于 ONNX Runtime 的生成式推理库；`onnx-genai-cli` 依赖其 Chat Template 与生成接口 |
| [`third-party/stb`](https://github.com/nothings/stb) | `31c1ad37456438565541f4919958214b6e762fb4` | 单文件图片解码库；`onnx-cli` 通过 `stb_image` 加载输入图片 |
...
项目所需的第三方源码通过 Git 子模块管理。选择以下任一方式完成初始化。
...
./scripts/init_submodules.sh -s stb                         # 仅初始化 stb 图片解码库
...
图片推理还需要 `onnx/vision_encoder_q4.onnx` 及其同名
`onnx_data` 权重文件。
...
`onnx-cli` 依赖 ONNX Runtime 共享库。请按照 ONNX Runtime 官方
...
构建产物为 `build/onnx-cli`。默认配置使用子模块目录 `third-party/onnxruntime` 及其 `build/Linux/RelWithDebInfo` 构建输出；CMake 会写入运行时库搜索路径，因此运行时无需设置 `LD_LIBRARY_PATH`。
...
| --- | --- |
| `--model DIR` | 模型根目录 |
| `--image FILE` | 图片文件；支持 `stb_image` 可解码的格式 |
...
## 8. 可选示例：Qwen3-0.6B（`onnx-genai-cli`）

`onnx-genai-cli` 基于 ONNX Runtime GenAI，与 `onnx-cli` 相互独立，默认不参与构建。
...
`onnx-genai-cli` 默认使用上述 CPU INT4 子目录。模型存放在其他位置或使用其他设备版本时，通过 `--model` 指向直接包含 `genai_config.json` 的子目录。

### 8.2 编译

`onnx-genai-cli` 依赖 ONNX Runtime GenAI 及其配套 ONNX Runtime。请按照 ONNX Runtime GenAI 官方
